...
func compiledVersion(t *testing.T) semver.Version {
	...
	cmd := exec.Command("go", "version", filepath.Join(testutils.MustGetRepoRoot(t), "gpupgrade"))
...
func goModVersion(t *testing.T) semver.Version {
	...
	contents, err := os.ReadFile(filepath.Join(testutils.MustGetRepoRoot(t), "go.mod"))
	if err != nil {
